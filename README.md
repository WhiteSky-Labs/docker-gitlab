...
- [Team](#team)
- [Issues](#issues)
- [Announcements](https://git.whiteskylabs.com/gitlab/docker-gitlab/issues)
...
- Be a part of the community and help resolve [Issues](https://git.whiteskylabs.com/gitlab/docker-gitlab/issues)

# Team

- Reynold Lariza ([reynold](https://git.whiteskylabs.com/reynold))
...
If using the latest docker version and/or disabling selinux does not fix the issue then please file a issue request on the [issues](https://git.whiteskylabs.com/gitlab/docker-gitlab/issues) page.
...
Automated builds of the image are available on [Dockerhub](https://hub.docker.com/r/wslph/gitlab) and is the recommended method of installation.


```bash
docker pull wslph/gitlab:latest
```
...
```bash
docker build -t wslph/gitlab git.whiteskylabs.com/gitlab/docker-gitlab
```
...
```bash
wget https://git.whiteskylabs.com/gitlab/docker-gitlab/blob/master/docker-compose.yml
```
...
docker run --name gitlab -d [OPTIONS] \
    wslph/gitlab:latest app:sanitize
```
...
| `NGINX_SET_REAL_IP_HEADER` | Set the value for `real_ip_header`. Can be `X-Real-IP` or `X-Forwarded-For` or `proxy_protocol`. Defaults to `X-Forwarded-For`. |
| `NGINX_SET_REAL_IP_FROM` | Set the value for `set_real_ip_from`. Defaults to `0.0.0.0/0`. |
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    wslph/gitlab:latest app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
	wslph/gitlab:latest app:rake db:setup
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    wslph/gitlab:latest app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    wslph/gitlab:latest app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    wslph/gitlab:latest app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    wslph/gitlab:latest app:rake gitlab:import:repos
```
...
> **Note**
>
> Upgrading to `wslph/gitlab:latest` from `wslph/gitlab:7.x.x` can cause issues. It is therefore required that you first upgrade to `wslph/gitlab:8.0.5-1` before upgrading to `wslph/gitlab:8.1.0` or higher.
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    wslph/gitlab:x.x.x app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -d [OPTIONS] wslph/gitlab:latest
```
